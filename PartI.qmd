...
We have only three education codes here, with 14, 16 and 'zzzOther'
representing a PhD, Bachelor's and Master's respectively. 
...
[Possibly the 'age' variable is birth year.  The data are from 1991, so
...
What about passage of the bar exam?  Does race play a role?
...
Looks pretty good, generally logit-shaped.  
...
Family income in this dataset is measured by quintiles.  So [It is often
mentioned, correctly, that testing for a 0 effect is equivalent to
checking whether the CI contains 0.  But this is missing the point of
the CI, which is to (a) give us an idea of the effect *size*, and (b) to
...
between race and undergraduate GPA to our above model. Let's fit two
models, one without interactions and one with, and do a prediction from
each.  For simplicity, let's use R's linear model function, **lm()**,
directly:
...
Indeed, the Black and white interaction terms are "significant," even
"very highly significant."  But that does mean we should use the more
complex model?

Recall that many machine learning algorithms, including standard linear
regression, use the estimated regression function value as its predicted
value.  So we see here that adding in an interaction term changed the
estimated value of the regession function by only about 0.02 out of a
40.23 baseline.  So, we may well prefer the simpler, no-interaction
model.
...
risk of recidivism is given in deciles, the value is 1,2,...,10, and we see
that the tool does indeed appear to be biased against Black defendants.
Though the effect is rather small, at most 0.6 of a decile point,  
...
of being Black, 0.4680, is actually larger than the estimated effect of
...
* It may cause overfitting, resulting in large standard errors.

* It is unwieldy, difficult to interpret.  Many treatments of these
  issues speak of a desire for a "parsimonious" model.

* There is a concern regarding partial duplication.  It 
  may be that, say, some pair of confounders suffices, with the rest 
  being highly correlated with this pair. Then adding 
  further confounders does nothing to further clarify 
...
[From the material in Section @sec-sig, it is clear that one way *not*
to choose C variables would be to do sigificance testing, $H_0: \beta_i
= 0$.]{.column-margin}
...
Abpit 21% pf the women had a Mast3r's degree, with a figure of 22% for
men, and so on.  The similar frequency values between men and women
suggests not using education as a confounder.
...
occupation 106 [Again, one might consider conducting formal statistical
inference in this comparison, calculating a p-value from a *chi-square
test*.  But again as we saw in @sec-sig. testing for model fit suffers from
the same problems as testing for nonzero effects.]{.column-margin} versus
females in that same occupation--a difference of approximately 11%.  The
difference in frequencies here is much greater than with the educationW
...
We should definitely consider using occupation as a confounder.
...
run the above code directly from R.  Try features such as move,
rotate, annotation display and so on.]{.column-margin} Of course,
visualizing a 3-dimensional scatter plot is more challenging than
...
some credence to our earlier analysis  that family income may confound
the relationship between race and LSAT score. Indeed, if we wish to
present the earlier analysis, say in a meeting, the graph would make for
a nice visual illustration.

Note however, that these graphs do not do much to answer the question of
whether, in this case, that relationship is substantial.  The formal
analysis we did earlier indicates that it is not.

Returning to the question of whether race has a substantial impact on
exam results, we can look at the density plot of LSAT scores for
different races.
...
selection* [A nice overview of such methodology, implementations in
R and Python, and problems that may arise, is given in [Parr *et
...
algorithm, we do two runs through the data, first with the original
dataset, and then with the V column changed through a random shuffling
of the values in that column. This may seem counterproductive at first,
but the point is that if the variable in question really is important,
our predictive accuracy should be compromised in that second run.  The
importance measure is thentaken to be the proportional increase in error
...
For instance, the singlevariables (excluding S) that predict Y well are
...
**loan_val**. We may wish to use these as our confounders.
...
Sept. 2004), researchera "tested" employers by sending CVs with
...
```{r}
data(lak)
dim(lak)  # number of rows and columns
names(lak)
```
...
age distribution in the two drug groups.  Thus, even if our data still did
...
Trying to guess important variables that we do not have is difficult, of
course. It depends crucially on domain expertise, and then dealing with
it statistically is difficult if not impossible. Nevertheless,
...
The basic idea here is quite simple. Consider our above example
comparing two medications, A and B, with Y being the response variable.
Instead of comparing the two sample means of Y, we form pairs of data
points, with an A group and B group member in each pair. These pairs
form our new data. (Not every one of the original data points will
necessarily now be represented, though some may appear more than once.)
...
earlier in this book estimate the conditional mean Y for a given X,
whereas matching methods estimate the unconditional mean of Y. The
interpretation of the latter may be problematic, since our matching
process itself may have changed the distribution of X. (By the way,
changing the distribution of X does not change the conditional mean of
Y given X, as long as the change doesn't affect Y.)

We then compute mean Y in both groups.
...
DAGS are usually just writeups of one's "gut feelings" as to what causes
what. They cannot be derived from our data.  More on this below, but
under very stringent assumptions, we can attempt to from a DAG from the
data.  Here is an example of that, using a function in the **bnlearn**
package, for which a **dsld* wrapper is available: 
...
wage income. In other words, no wage gap!  This is in stark contrast to
what **dsldLinear** suggests:
...
causal concepts helpful in their thought processes regarding the data.
...
harmful in CI contexts as in general observational data methods.
...
long been used by statisticians. There are many R packages that
implement them.

:::

We referred to the quantities $\beta_i$ in the linear and logit models
as "parameters."  These models are thus called *parametric* models.  In
each case, the regression function is modeled as linear, "S
curve-shaped" and so on, that can be described with just a few
parameters.

But assuming, say, that our  regression function is approximately
linear.  It would be nice to have available methods that don't rely on
such assumptions.
...
quantity in our no-interactions linear model.  Nor are standard
...
we do here, so they can be quite useful in our context of adjusting for
...
graph that is *too* smooth. In the extreme, using just one bin will
produce a graph that is perfectly smooth but completely meaningless.
...
using the one that best predicts in a holdout set, described below.
Once we've settled on k, we might use the full dataset, no holdout. 
...
set of 1000 rows. The given algorithm, say kNN, will fit the training
...
Nonparametric methods are inherently of the "with interactions" type.
Thus there is no single value of the gender wage gap; instead, the gap
will have different values at different X's. Thus, as with the
with-interactions case for linear models, the software reports the
estimated wage gap here at a few comparison cases.
...
having worked 52 weeks, women's average wage was 48,914 and for men it
was 61,156.
...
assumptions on the form of the regression function.  
...
famous example involving vertebral disease.  Y is categorical, with
values NO (normal), SL and DH.  To predict a new case, we look at the X
variables (which have the nondescript names V1 through V6) one at a
time, and branch through the tree accordingly.
...
The tree is also giving us the conditional probabilities. For instance,
the node at the far bottom left says that if a new case has V6 < 16 and
V4 < 28, the estimated probabilities of this patient being of class DH,
NO and SL are 0.74, 0.25 and 0.00, respectively. The numbers are the
proportions of cases in our training set that fell into that node.
(The 11% figure in that node is the proportion of all training set cases
that fell into that node.)

RF typically has several hyperparameters, e.g. a limit on the number of
levels in a tree, and the number of trees.
...
in the root node in the picture, are also random. They are obtained
through formulas that depend on the data in complex (though not terribly
deep) ways.

This means there is quite a bit of randomness in a tree, so RF hedges
its bets by generating many trees from our dataset, and then predicts
new cases by averaging the predictions for that case from the individual
trees. Let's make that concrete via an example. Let's run RF directly
from the **qeML** function (to enable predictions), and then predict a
hypothetical new case of a 32-year-old woman etc.:

```{R}
set.seed(9999)
z <- qeRF(svcensus,'wageinc',holdout=NULL)
predict(z,data.frame(age=32,educ='zzzOther',occ='102',
   wkswrkd=52,gender='female'))
```

RF generated 500 trees (the default in **qeRF**), generating 500
predictions for this hypothetical person, and averaged them.

As usual, the **dsld** wrapper adds S comparison information:

```{R}
set.seed(9999)
w <- dsldML(svcensus,'wageinc','gender',
   qeMLftnName='qeRF')  # default values for hyperparameters
w
```

The code generated 500 trees, the default value, from our training set.
