...
The relation of mean Y to the X, C (or O) and S variables is formally called
...
women of the same age, but women tend to be old, the gender and age
...
S.  So, the call here says, "Plot smoothed wage income against age, for each
gender."  
...
reported standard errors, but this can be adjusted via the so-called
...
Suppose that within each gender, X has a normal (Gaussian) distribution,
the familiar "bell-shaped" curve, with the same standard deviation for
each gender.  Then it turns out that one can show mathematically that
...
The values $\beta_0$ and $\beta_1$ actually are expressions involving
quantities such as the per-gender mean incomes, but the point is that in
the end the probability has a linear component to it, $\beta_0 + \beta_1
income$.
...
$\infty$.]{.column-margin} for any age level, albeit on this much
less-interpretable log scale.  
...
prediction accuracy (**testAcc** values) on the holdout set.   The RF
version looks slightly better, but for full thoroughness, we would need
to try different hyperparameter values, and so on.  Short of going
through all that, the two models are similar.
...
[Recall that the **fam_inc** variable is measured in terms of quintiles,
1 through 5.  This is the reason behind the seemingly-odd alignment of,
say, the Black and Latino curves.]{.column-margin} White students have
larger peaks at income levels 3, 4, and 5, indicating that a larger
proportion of white students are in the higher income brackets than
non-white students. Conversely, a larger proportion of black and Latino
