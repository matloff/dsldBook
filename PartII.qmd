...
* Here in Part II, prediction of Y is our central goal, rather than
  effect assessment.  We will either omit or reduce the impact of S,
...
As with choosing confounders in Part I, the analyst may simply choose to
include all possible candidate O variables. But then we would risk
allowing S to have major influence on Y, exactly what we are trying to
avoid. Or we might choose the proxies based on domain expertise.  But a
more formal approach may involve correlation.  The function
**dsldOHunting** calculates the correlations between S and possible
proxies O.
