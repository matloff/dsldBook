...
### The **dsldFrrm** and **dsldFgrrm** function

These are wrappers to functions in the **fairml** package, based on [a
...
thus reducing its influence.

These functions aim to address the fairness issue through an argument
**unfairness**, a number in (0,1].  The smaller the value, the fairer
the fit. Note that unlike
...
The first value, 0.1, gave an estimate
coefficient for Caucasian of -0.37, while using 0.01 reduced this to
-0.03.  (Note that the base for the dummy variables is apparently
African-American.] There seems to be almost no effect on our possible
proxies, age and priors count.
