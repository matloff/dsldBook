...
We wish to predict Y from X and O, either omitting S or reducing its
impact, but with concern that we may be indirectly using S via O.
...
* Here in Part II, prediction of Y is our central goal, rather than
  effect assessment.  We will either omit or reduce the impact of SS,
  relying our prediction fully on X and partly on O.  The variables O
  are related to S; the stronger the relation of an O variable to S, the
  less weight we will put on that variable in predicting Y.

### Deciding proxies  

As with choosing confounders in Part I, the analyst may simply choose
all possible candidate O variables. But then we would risk allowing S to
have major influence on Y, exactly what we are trying to avoid. Or
we might choose the proxies based on domain expertise.  But a more formal
approach may involve correlation.  The function **dsldOHunting**
calculates the correlations between S and possible proxies O.
